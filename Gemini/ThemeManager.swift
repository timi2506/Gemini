...
class ThemeManager: ObservableObject {
    ...
    static var baseApplicationSupportDirectory: URL {
        FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: "group.timi2506.Gemini") ??         FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
    }
    ...
    func saveWallpaper(pngData: Data, name: String) throws {
        ...
        UserDefaults(suiteName: "group.timi2506.Gemini")!.set(newID.uuidString, forKey: "selectedWallpaper")
    }
...
struct WallpaperPreviewView: View {
    ...
    @StateObject var manager = ThemeManager.shared
    var config: SwiftyCropConfiguration {
        ...
        if #available(iOS 26, *) {
            return SwiftyCropConfiguration(usesLiquidGlassDesign: true, zoomSensitivity: 10, rectAspectRatio: aspectRatio)
        } else {
            return SwiftyCropConfiguration(zoomSensitivity: 10, rectAspectRatio: aspectRatio)
        }
    }
    var body: some View {
        SwiftyCropView(imageToCrop: UIImage(data: wall.data) ?? UIImage(systemName: "apple.logo")!, maskShape: MaskShape.rectangle, configuration: config) { croppedImage in
            if let croppedImage, let pngData = croppedImage.pngData() {
                do {
                    try manager.saveWallpaper(pngData: pngData, name: "")
                    imageData = nil
                } catch {
                    print(error.localizedDescription)
                }
